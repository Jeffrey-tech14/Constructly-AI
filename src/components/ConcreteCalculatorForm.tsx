  ...
  export default function ConcreteCalculatorForm({ quote, setQuote }: Props) {
    ...
    const pushRowsDebounced = useCallback((nextRows: ConcreteRow[]) => {
    ...
    const updateRow = useCallback(
      <K extends keyof ConcreteRow>(id: string, key: K, value: ConcreteRow[K]) => {
        setRows((prev) => {
    ...
    const results: ConcreteResult[] = useMemo(() => rows.map((r) => calculateConcrete(r)), [rows]);
    ...
    useEffect(() => {
      ...
      if (results.length === 0) {
        ...
        const hasItems = Array.isArray(quote?.concrete_materials) && quote.concrete_materials.length;
        if (hasItems) {
      ...
      const currItems = Array.isArray(quote?.concrete_materials) ? quote.concrete_materials : [];
      const same = JSON.stringify(currItems) === JSON.stringify(nextItems);
      if (!same) {
        setQuote((prev: any) => ({ ...prev, concrete_materials: nextItems }));
      }
    }, [results, cementMat, sandMat, ballastMat, totals, setQuote, quote?.concrete_materials]);
            ...
            <div key={row.id} className="p-4 border rounded-lg space-y-2">
              ...
              {result && (
                <div className="mt-2 text-sm">
                  <p>
                    <b>Volume:</b> {result.volumeM3.toFixed(2)} m³
                  </p>
                  <p>
                    <b>Cement:</b> {result.cementBags.toFixed(1)} bags —{" "}
                    <b>Ksh {Math.round(result.cementBags * (cementMat?.price || 0))}</b>
                  </p>
                  <p>
