...
const DashboardSettings = () => {
    ...
    const { loading: settingsLoading, equipmentTypes, equipmentRates, transportRates, additionalServices, serviceRates, updateEquipmentRate, updateTransportRate, updateServiceRate, } = useUserSettings();
      ...
      <Tabs defaultValue="equipment" className="w-full">
        ...
        <TabsContent value="equipment" className="space-y-4">
          <Card className="">
            ...
            return (<div key={equipmentType.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                      <h4 className="font-medium">{equipmentType.name}</h4>
                      <p className="text-sm text-muted-foreground">
                        Current: KSh {currentRate.toLocaleString()}/day
                      </p>
                      {equipmentType.description && (<p className="text-xs text-muted-foreground">
                          {equipmentType.description}
                        </p>)}
                    </div>
                    <div className="flex items-center space-x-2">
                      <Input type="number" min="0" placeholder={currentRate.toLocaleString()} className="w-32" onChange={(e) => setTempValues({
                    ...tempValues,
                    [`equipment-${equipmentType.id}`]: parseFloat(e.target.value) || 0,
                })}/>
                      <Button size="sm" className="text-white" onClick={() => handleUpdateEquipmentRate(equipmentType.id, tempValues[`equipment-${equipmentType.id}`] ||
                    currentRate)} disabled={loading}>
                        Update
                      </Button>
                    </div>
                  </div>);
        ...
        <TabsContent value="transport" className="space-y-4">
          <Card className="">
            ...
            return (<div key={region} className="p-4 border rounded-lg">
                    ...
                    <Button className="mt-4 text-white" size="sm" onClick={() => handleUpdateTransportRate(region, tempValues[`transport-km-${region}`] || costPerKm, tempValues[`transport-base-${region}`] || baseCost)} disabled={loading}>
        ...
        <TabsContent value="services" className="space-y-4">
          <Card className="">
            ...
            return (<div key={service.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
                      <h4 className="font-medium">{service.name}</h4>
                      <p className="text-sm text-muted-foreground">
                        Current: KSh {currentPrice.toLocaleString()}
                      </p>
                      {service.description && (<p className="text-xs text-muted-foreground">
                          {service.description}
                        </p>)}
                    ...
                    <div className="flex items-center space-x-2">
                      <Input type="number" min="0" className="w-32" placeholder={currentPrice.toLocaleString()} onChange={(e) => setTempValues({
                    ...tempValues,
                    [`service-${service.id}`]: parseFloat(e.target.value) || 0,
                })}/>
                      <Button size="sm" className="text-white" onClick={() => handleUpdateServiceRate(service.id, tempValues[`service-${service.id}`] || currentPrice)} disabled={loading}>
                        Update
                      </Button>
                    </div>
                  </div>);
