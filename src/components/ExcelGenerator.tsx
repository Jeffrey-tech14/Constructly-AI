...
export const generateQuoteExcel = async ({ quote, isClientExport = false, }: {
    ...
    if (!isClientExport) {
        summaryData.push(["Overhead", quote.overhead_amount], ["Contingency", quote.contingency_amount], ["Permit Cost", quote.permit_cost], ["Profit Margin", quote.profit_amount]);
    }
    ...
    if (quote.boq_sections?.length) {
        ...
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(boqData), "Bill of Quantities");
    }
    if (quote.materials?.length && !isClientExport) {
        ...
        XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(materialsData), "Materials Schedule");
    }
