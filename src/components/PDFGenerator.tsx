...
import { AdvancedMaterialExtractor, CategorizedMaterial, MaterialSchedule, } from "@/utils/advancedMaterialExtractor";
import { ConsolidatedMaterial, MaterialConsolidator, } from "@/utils/materialConsolidator";
...
const PDFGenerator: React.FC<PDFGeneratorProps> = ({ boqData, projectInfo, preliminariesData = [], materialSchedule, calculationSummary, equipmentItems = [], additionalServices = [], subcontractors = [], addons = [], transportCost = 0, contractType = "full_contract", profit = 0, contingency_amount = 0, overhead_amount = 0, labour = 0, permits = 0, isClientExport = false, }) => {
    ...
    const calculateEquipmentTotal = useMemo(() => equipmentItems.reduce((sum, item) => sum + item.total_cost, 0), [equipmentItems]);
    const calculateServicesTotal = useMemo(() => additionalServices.reduce((sum, service) => sum + service.price, 0), [additionalServices]);
    const calculateSubcontractorsTotal = useMemo(() => subcontractors.reduce((sum, sub) => sum + sub.total, 0), [subcontractors]);
    const calculateAddonsTotal = useMemo(() => addons.reduce((sum, addon) => sum + addon.price, 0), [addons]);
    ...
    const renderAdditionalCostDetails = () => {
        ...
        <View style={styles.table}>
          ...
          {!isClientExport && (<View style={styles.tableRow}>
              <Text style={[styles.tableColDescription, { width: "70%" }]}>
                Profit:
              </Text>
              <Text style={[styles.tableColAmount, { width: "30%" }]}>
    ...
    const renderEquipmentPage = (equipment: EquipmentItem[]) => {
        ...
        if (isClientExport)
            return null;
        return (<Page key="equipment-page" style={styles.page}>
        <CompanyHeader />
    ...
    const renderSubcontractorsPage = (subcontractors: Subcontractor[]) => {
        ...
        if (isClientExport)
            return null;
        return (<Page key="subcontractors-page" style={styles.page}>
        <CompanyHeader />
    ...
    const renderAddonsPage = (addons: Addon[]) => {
        if (!addons || addons.length === 0)
            return null;
        return (<Page key="addons-page" style={styles.page}>
        <CompanyHeader />
    ...
    const renderServicesPage = (services: AdditionalService[]) => {
        if (!services || services.length === 0)
            return null;
        return (<Page key="services-page" style={styles.page}>
        <CompanyHeader />
    ...
    const renderMaterialRow = (material: ConsolidatedMaterial) => (<View style={styles.materialScheduleRow}>
      <Text style={styles.materialScheduleColItem}>{material.itemNo}</Text>
      <View style={{ flex: 1 }}>
        <Text style={styles.materialScheduleColDescription}>
          {material.description}
        </Text>
        {material.category && (<Text style={styles.materialSource}>From: {material.category}</Text>)}
      </View>
    ...
    const renderConsolidatedMaterials = (materials: ConsolidatedMaterial[]) => {
        ...
        return Object.entries(materialsByCategory).flatMap(([category, categoryMaterials]) => {
            const itemChunks = chunkArray(categoryMaterials, 18);
            return itemChunks.map((chunk, chunkIndex) => (<Page key={`materials-${category}-${chunkIndex}`} size="A4" style={styles.page}>
            <CompanyHeader />
            ...
            <View style={styles.materialScheduleTable}>
              ...
              {chunkIndex === itemChunks.length - 1 && (<View style={styles.sectionTotalRow}>
                  <Text style={styles.sectionTotalLabel}>
                    Total for {category}:
                  </Text>
                  <Text style={styles.sectionTotalValue}>
                    {formatCurrency(categoryMaterials.reduce((sum, m) => sum + (m.amount || 0), 0))}
                  </Text>
                </View>)}
            </View>
            ...
            {category ===
                    Object.keys(materialsByCategory)[Object.keys(materialsByCategory).length - 1] &&
                    chunkIndex === itemChunks.length - 1 && (<View style={styles.grandTotalRow}>
    ...
    const calculatePreliminariesTotal = (): number => {
        if (!Array.isArray(preliminariesData))
            return 0;
    ...
    const preliminariesTotal = useMemo(() => calculatePreliminariesTotal(), [preliminariesData]);
    const calculateComprehensiveTotal = useMemo(() => {
        const baseTotal = preliminariesTotal +
    ...
    const CompanyHeader = () => (<View style={styles.companyHeader}>
      {projectInfo.logoUrl ? (<Image style={styles.logoContainer} source={projectInfo.logoUrl}/>) : projectInfo.companyName ? (<View style={styles.logoContainer}>
      ...
      {(projectInfo.companyName || projectInfo.companyTagline) && (<View style={styles.companyInfo}>
          {projectInfo.companyName && (<Text style={styles.companyName}>{projectInfo.companyName}</Text>)}
          {projectInfo.companyTagline && (<Text style={styles.companyTagline}>
    ...
    const renderEquipmentSummary = (equipment: EquipmentItem[]) => {
        ...
        <View style={styles.table}>
          ...
          {equipment.map((item, index) => (<View key={index} style={styles.tableRow}>
              <Text style={[styles.tableColDescription, { width: "70%" }]}>
                {item.name}
              </Text>
              <Text style={[styles.tableColAmount, { width: "30%" }]}>
          ...
          <View style={[styles.tableRow, styles.headerRow]}>
            ...
            <Text style={[styles.tableColAmount, { width: "30%" }]}>
              {formatCurrency(equipment.reduce((sum, item) => sum + item.total_cost, 0))}
            </Text>
          </View>
        </View>
      </View>);
    ...
    const renderServicesSummary = (services: AdditionalService[]) => {
        ...
        <View style={styles.table}>
          ...
          {services.map((service, index) => (<View key={index} style={styles.tableRow}>
              <Text style={[styles.tableColDescription, { width: "70%" }]}>
                {service.name}
              </Text>
              <Text style={[styles.tableColAmount, { width: "30%" }]}>
    ...
    const renderSubcontractorsSummary = (subcontractors: Subcontractor[]) => {
        ...
        <View style={styles.table}>
          ...
          {subcontractors.map((sub, index) => (<View key={index} style={styles.tableRow}>
          ...
          <View style={[styles.tableRow, styles.headerRow]}>
            ...
            <Text style={[styles.tableColAmount, { width: "20%" }]}>
              {formatCurrency(subcontractors.reduce((sum, sub) => sum + sub.total, 0))}
            </Text>
          </View>
        </View>
      </View>);
    ...
    const renderAddonsSummary = (addons: Addon[]) => {
        ...
        <View style={styles.table}>
          ...
          {addons.map((addon, index) => (<View key={index} style={styles.tableRow}>
              <Text style={[styles.tableColDescription, { width: "70%" }]}>
                {addon.name}
              </Text>
              <Text style={[styles.tableColAmount, { width: "30%" }]}>
          ...
          <View style={[styles.tableRow, styles.headerRow]}>
            ...
            <Text style={[styles.tableColAmount, { width: "30%" }]}>
              {formatCurrency(addons.reduce((sum, addon) => sum + addon.price, 0))}
            </Text>
          </View>
        </View>
      </View>);
    ...
    return (<Document>
      ...
      <Page size="A4" style={styles.prelimPage}>
        ...
        <View style={styles.projectInfoContainer}>
          ...
          {projectInfo.consultant && (<View style={styles.projectInfoRow}>
              <Text style={styles.projectInfoLabel}>Consultant:</Text>
              <Text style={styles.projectInfoValue}>
                {projectInfo.consultant}
              </Text>
            </View>)}
          {projectInfo.contractor && (<View style={styles.projectInfoRow}>
      ...
      {preliminariesData.length > 0 && (<Page size="A4" style={styles.prelimPage}>
          <CompanyHeader />
          {preliminariesData.map((items, index) => (<Text style={styles.prelimTitle}>{items.title}</Text>))}
          ...
          <View style={styles.table}>
            <View style={[styles.tableRow, styles.tableHeaderRow]}>
              ...
              <Text style={[styles.tableColHeaderDescription, { width: "70%" }]}>
                DESCRIPTION
              </Text>
              <Text style={[styles.tableColHeader, { width: "20%" }]}>
                AMOUNT (KSh)
            ...
            {preliminariesData.map((prelim, index) => prelim.items.map((item, subIndex) => {
                if (item.isHeader) {
                    return (<View style={[styles.tableRow, { backgroundColor: "#F3F4F6" }]} key={`prelim-header-${index}-${subIndex}`}>
                ...
                return (<View style={styles.tableRow} key={`prelim-${index}-${subIndex}`}>
                    <Text style={[styles.tableCol, { width: "10%" }]}>
                      {item.itemNo}
                    </Text>
                    <Text style={[styles.tableColDescription, { width: "70%" }]}>
      ...
      {nonEmptySections.map((section, sectionIndex) => {
            ...
            return itemChunks.map((chunk, chunkIndex) => (<Page size="A4" style={styles.page} key={`section-${sectionIndex}-chunk-${chunkIndex}`}>
            <CompanyHeader />

            
            {chunkIndex === 0 && (<View style={styles.section}>
                <Text style={styles.sectionTitle}>{section.title}</Text>
              </View>)}

            <View style={styles.table}>
              
              {chunkIndex === 0 && (<View style={styles.tableHeaderRow}>
                  <Text style={[styles.tableColHeader, { width: "12%" }]}>
                    ITEM
                  </Text>
                  <Text style={[styles.tableColHeaderDescription, { width: "48%" }]}>
                    DESCRIPTION
                  </Text>
              ...
              {chunk.map((item, itemIndex) => {
                    if (item.isHeader) {
                        return (<View style={[styles.tableRow, { backgroundColor: "#F3F4F6" }]} key={`header-${sectionIndex}-${chunkIndex}-${itemIndex}`}>
                    ...
                    return (<View style={styles.tableRow} key={`item-${sectionIndex}-${chunkIndex}-${itemIndex}`}>
                    <Text style={[styles.tableCol, { width: "12%" }]}>
                      {item.itemNo}
                    </Text>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            ...
            {preliminariesData.length > 0 && (<View style={styles.tableRow}>
                <Text style={[styles.tableColDescription, { width: "70%" }]}>
                  Preliminaries:
                </Text>
                <Text style={[styles.tableColAmount, { width: "30%" }]}>
            ...
            {consolidatedMaterials.length > 0 && (<View style={styles.tableRow}>
                <Text style={[styles.tableColDescription, { width: "70%" }]}>
                  Materials:
                </Text>
                <Text style={[styles.tableColAmount, { width: "30%" }]}>
                  {formatCurrency(consolidatedMaterials.reduce((sum, m) => sum + m.amount, 0))}
                </Text>
              </View>)}

            
            {equipmentItems.length > 0 && (<View style={styles.tableRow}>
                <Text style={[styles.tableColDescription, { width: "70%" }]}>
                  Equipment:
                </Text>
                <Text style={[styles.tableColAmount, { width: "30%" }]}>
                  {formatCurrency(equipmentItems.reduce((sum, item) => sum + item.total_cost, 0))}
                </Text>
              </View>)}

            
            {additionalServices.length > 0 && (<View style={styles.tableRow}>
                <Text style={[styles.tableColDescription, { width: "70%" }]}>
                  Additional Services:
                </Text>
                <Text style={[styles.tableColAmount, { width: "30%" }]}>
                  {formatCurrency(additionalServices.reduce((sum, service) => sum + service.price, 0))}
                </Text>
              </View>)}

            
            {subcontractors.length > 0 && (<View style={styles.tableRow}>
                <Text style={[styles.tableColDescription, { width: "70%" }]}>
                  Subcontractors:
                </Text>
                <Text style={[styles.tableColAmount, { width: "30%" }]}>
        ...
        {calculationSummary && (<View style={styles.section}>
            <Text style={styles.sectionTitle}>ADDITIONAL COSTS & MARGINS</Text>

            <View style={styles.table}>
              
              {calculationSummary.overhead_amount > 0 && (<View style={styles.tableRow}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Overhead (
              ...
              {calculationSummary.contingency_amount > 0 && (<View style={styles.tableRow}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Contingency (
              ...
              {calculationSummary.labor_cost > 0 && (<View style={styles.tableRow}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Labor(
              ...
              {contractType && (<View style={styles.tableRow}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Contract type:
                  </Text>
                  <Text style={[styles.tableColAmount, { width: "30%" }]}>
              ...
              {calculationSummary.permit_cost > 0 && (<View style={styles.tableRow}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Permit & Fees:
                  </Text>
                  <Text style={[styles.tableColAmount, { width: "30%" }]}>
              ...
              {calculationSummary.transport_costs > 0 && (<View style={styles.tableRow}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Transport:
                  </Text>
                  <Text style={[styles.tableColAmount, { width: "30%" }]}>
              ...
              {calculationSummary.profit_amount > 0 && (<View style={[styles.tableRow]}>
                  <Text style={[styles.tableColDescription, { width: "70%" }]}>
                    Profit Margin (
        ...
        <View style={[styles.section, { marginTop: 10 }]}>
          <View style={[styles.table, { border: "2pt solid #3B82F6" }]}>
            ...
            { width: "30%", color: "#FFFFFF", fontWeight: "bold" },
                ...
                {formatCurrency(calculationSummary?.total_amount ||
            preliminariesTotal +
                consolidatedMaterials.reduce((sum, m) => sum + m.amount, 0) +
                equipmentItems.reduce((sum, item) => sum + item.total_cost, 0) +
                additionalServices.reduce((sum, service) => sum + service.price, 0) +
        ...
        <View style={[styles.section, { marginTop: 15 }]}>
          ...
          <Text style={[styles.prelimContent, { fontSize: 9 }]}>
            ...
            {calculationSummary &&
            ` • Includes ${calculationSummary.percentages?.[0]?.profit || 0}% profit margin`}
            {calculationSummary &&
            calculationSummary.percentages?.[0]?.contingency > 0 &&
