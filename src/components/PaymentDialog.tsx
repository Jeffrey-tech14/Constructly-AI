...
export default function PaymentAction() {
    ...
    return (<div className="mt-20 flex items-center justify-center">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="max-w-lg w-full mx-4 text-center space-y-4">
        ...
        {paymentStatus === "processing" || isLoading ? (<div className="space-y-4 py-8">
            <Loader2 className="w-12 h-12 animate-spin text-blue-500 mx-auto"/>
            <p className="font-semibold">Processing your payment...</p>
          </div>) : paymentStatus === "success" ? (<div className="space-y-4 py-8">
          ...
          </div>) : paymentStatus === "error" ? (<div className="space-y-4 py-8">
            <XCircle className="w-16 h-16 text-red-500 mx-auto"/>
            <h2 className="text-2xl font-semibold">Payment Failed</h2>
            <p className="text-muted-foreground">
              Something went wrong. Please try again.
            </p>
            <Button onClick={() => setPaymentStatus("idle")} className="w-full text-white">
              Try Again
            </Button>
          </div>) : (<div className="space-y-4 mt-10 text-left">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-sm font-semibold">
                Email Address
              </Label>
              <Input id="email" type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} disabled={isLoading}/>
            </div>

            {!transactionRef ? (<Button onClick={setupCardPayment} disabled={paymentAmount <= 0 || !email || isSettingUpCard} className="w-full bg-gradient-to-r from-blue-600 to-indigo-700 text-white hover:opacity-90">
                {isSettingUpCard ? (<>
                    <Loader2 className="w-4 h-4 animate-spin mr-2"/>
                    Setting up payment...
                  </>) : (`Set up payment - KES ${paymentAmount}`)}
              </Button>) : (<Button onClick={handlePaystackPayment} disabled={isLoading} className="w-full bg-gradient-to-r from-blue-600 to-indigo-700 text-white hover:opacity-90">
                Pay KES {paymentAmount}
              </Button>)}

            {paymentAmount > 0 && (<div className="p-4 rounded-lg border bg-muted mt-8">
