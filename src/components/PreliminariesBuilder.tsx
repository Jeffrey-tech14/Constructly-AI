...
const PreliminariesBuilder = ({ quoteData, onPreliminariesUpdate, onSaveToQuote, }: PreliminariesBuilderProps) => {
    const { user } = useAuth();
    ...
    const calculateSectionTotal = (items: PrelimItem[]): number => items.reduce((sum, i) => (i.isHeader ? sum : sum + (i.amount || 0)), 0);
    const calculateGrandTotal = (): number => sections.reduce((sum, s) => sum + calculateSectionTotal(s.items), 0);
    const updateSectionTitle = (sectionIndex: number, title: string) => {
        const newSections = [...sections];
        newSections[sectionIndex].title = title;
        updateSections(newSections);
    };
    const updateItem = (sectionIndex: number, itemIndex: number, field: keyof PrelimItem, value: any) => {
        const newSections = [...sections];
    ...
    const addItem = (sectionIndex: number) => {
        const newSections = [...sections];
        const newItemNo = `A${newSections[sectionIndex].items.filter((i) => !i.isHeader).length + 1}`;
    ...
    const addHeader = (sectionIndex: number) => {
        const newSections = [...sections];
        const newItemNo = `HDR-${newSections[sectionIndex].items.filter((i) => i.isHeader).length + 1}`;
        newSections[sectionIndex].items.push({
            itemNo: newItemNo,
    ...
    return (<div className="space-y-6">
      ...
      {sections.map((section, sectionIndex) => (<Card key={sectionIndex} className="">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={() => removeSection(sectionIndex)} disabled={sections.length <= 1}>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                ...
                const isEditing = editingItem?.sectionIndex === sectionIndex &&
                    editingItem?.itemIndex === itemIndex;
                return (<TableRow key={`${sectionIndex}-${itemIndex}-${item.itemNo}`} className={item.isHeader
                        ? "bg-gray-100 dark:bg-gray-700 rounded-lg font-semibold"
                        : ""}>
                      <TableCell>{item.isHeader ? "" : item.itemNo}</TableCell>
                      <TableCell>
                        {isEditing ? (<Input value={item.description} onChange={(e) => updateItem(sectionIndex, itemIndex, "description", e.target.value)}/>) : (<span>{item.description}</span>)}
                      </TableCell>
                      <TableCell>
                        {item.isHeader ? ("") : isEditing ? (<Input type="number" value={item.amount || 0} onChange={(e) => updateItem(sectionIndex, itemIndex, "amount", e.target.value)}/>) : (`KSh ${(item.amount || 0).toLocaleString()}`)}
                      </TableCell>
                      <TableCell>
                        <div className="flex space-x-2">
                          <Button variant="outline" size="sm" onClick={() => toggleEdit(sectionIndex, itemIndex)}>
                ...
                {section.items.length > 0 && (<TableRow className="bg-muted/50 font-semibold">
      ...
      {sections.length > 0 && sections.some((s) => s.items.length > 0) && (<Card className="bg-primary/10 border-primary dark:border-blue-300">
          <CardHeader>
