...
const ProfilePictureUpload = ({ currentAvatarUrl, onUploadComplete, onCancel, }: ProfilePictureUploadProps) => {
    const { user } = useAuth();
    ...
    return (<Card className="w-full max-w-md">
      <CardContent className="pt-6">
        ...
        {!imgSrc ? (<div className="flex flex-col items-center space-y-4">
            {currentAvatarUrl && (<div className="relative">
                <img src={currentAvatarUrl} alt="Current profile" className="h-32 w-32 rounded-full object-cover"/>
              </div>)}
            <input type="file" ref={fileInputRef} onChange={onSelectFile} accept="image/*" className="hidden"/>
            <Button className="text-white" onClick={() => fileInputRef.current?.click()}>
            ...
            <div className="flex justify-end space-x-2">
              <Button variant="outline" onClick={() => setImgSrc(null)} disabled={isUploading}>
                Cancel
              </Button>
              <Button onClick={handleUpload} className="text-white" disabled={!completedCrop || isUploading}>
                {isUploading ? (<Loader2 className="h-4 w-4 mr-2 animate-spin"/>) : (<Crop className="h-4 w-4 mr-2"/>)}
