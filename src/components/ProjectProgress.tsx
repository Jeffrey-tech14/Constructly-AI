...
import { CheckCircle, Clock, Play, Pause, Pencil, CalendarIcon, } from "lucide-react";
...
const ProjectProgress = ({ quoteId, quoteName, onQuoteUpdated, }: ProjectProgressProps) => {
    const { user } = useAuth();
    ...
    return (<Card>
      <CardHeader className={`${getBackColor(progressData.status)} rounded-t-lg`}>
      ...
      <CardContent className={`${getBackColor(progressData.status)} space-y-6 rounded-b-lg`}>
        <div>
          <div className="flex items-center justify-between mb-2">
            <Label className="text-white">Progress Percentage</Label>
            <span className={`sm:text-2xl text-lg font-bold hover:bg-transparent ${getStatusColor(progressData.status)} bg-transparent`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Select value={progressData.status} onValueChange={(value: any) => setProgressData((prev) => ({ ...prev, status: value }))}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Input id="percentage" type="number" min="0" max="100" value={progressData.progress_percentage} onChange={(e) => setProgressData((prev) => ({
            ...prev,
        ...
        <div className="flex flex-col space-y-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={`w-[240px] justify-start text-left font-normal ${!progressData.milestone_date && "text-muted-foreground"}`}>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={progressData.milestone_date
            ? new Date(progressData.milestone_date)
            : undefined} onSelect={(date) => setProgressData((prev) => ({
            ...prev,
        ...
        <div>
          ...
          <Textarea id="notes" placeholder="Add notes about current progress, challenges, or next steps..." value={progressData.notes} onChange={(e) => setProgressData((prev) => ({ ...prev, notes: e.target.value }))} rows={4}/>
        </div>

        <Button onClick={updateProgress} disabled={updating} className="w-full text-white">
