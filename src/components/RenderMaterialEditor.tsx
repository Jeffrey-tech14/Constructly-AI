...
export default function renderMaterialEditor(material, tempValues, setTempValues, handleSave, materialBasePrices,
    userMaterialPrices,
    regionalMultipliers, userRegion, getEffectiveMaterialPrice) {
  ...
  if (material.name === "Rebar") {
    ...
    <Collapsible>
        <CollapsibleTrigger className="flex animate-fade-in items-center justify-between w-full p-2 border rounded-lg bg-primary/20">
            <span className="text-sm text-gray-500 dark:text-gray-300">{material.name}</span>
        </CollapsibleTrigger>
...
if (material.name === "Bricks" || material.name.includes("Block")) {
          ...
          {effectivePrice.type.map((block, idx) => {
            ...
            return (
                ...
                <span className="ml-2 text-xs text-gray-500">
                    {block.dimensions_m.length}×
                    {block.dimensions_m.height}×
  ...
  // Doors + Windows (similar structure)
if (material.name === "Doors" || material.name === "Windows") {
  const userOverride = userMaterialPrices.find(
    (p) => p.material_id === material.id && p.region === userRegion
  );
