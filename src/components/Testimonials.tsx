...
export function TestimonialsSection() {
    ...
    return (<section id="testimonials" className="bg-transparent">
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        <div className="max-w-5xl mx-auto relative mb-20 mt-10">
          ...
          <button onClick={prevTestimonial} className="absolute top-1/2 -left-4 md:-left-12 transform -translate-y-1/2 z-10 w-12 h-12 rounded-full shadow-xl flex items-center justify-center transition-all border bg-white text-gray-600 border-gray-200 hover:text-blue-600 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:text-primary" aria-label="Previous testimonial" disabled={testimonials.length <= 1}>
            <ChevronLeft />
          </button>

          
          <button onClick={nextTestimonial} className="absolute top-1/2 -right-4 md:-right-12 transform -translate-y-1/2 z-10 w-12 h-12 rounded-full shadow-xl flex items-center justify-center transition-all border bg-white text-gray-600 border-gray-200 hover:text-blue-600 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700 dark:hover:text-primary" aria-label="Next testimonial" disabled={testimonials.length <= 1}>
            <ChevronRightIcon />
          </button>

          
          <div className="relative h-auto overflow-hidden">
            {testimonials.length > 0 ? (<AnimatePresence mode="wait">
                <motion.div key={activeIndex} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.5 }} className={`rounded-2xl shadow-xl overflow-hidden border`}>
              <div className="rounded-2xl shadow-xl overflow-hidden border bg-white border-gray-100 dark:bg-gray-800 dark:border-gray-700">
                <div className="grid grid-cols-1 lg:grid-cols-3">
                  ...
                  <div className="lg:col-span-2 p-8 md:p-10">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between">
                      ...
                      <div className="flex items-center">
                        {[...Array(5)].map((_, i) => (<Star key={i} className={`text-xl ${i < currentTestimonial.rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`}/>))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-8 md:p-10 border-t lg:border-t-0 lg:border-l bg-gradient-to-br from-blue-50 to-white border-gray-200 dark:from-gray-700 dark:to-gray-800 dark:border-gray-700">
                    <h3 className="text-lg font-semibold mb-6">Key Achievements</h3>
                    <div className="grid grid-cols-2 gap-4">
                      {currentTestimonial.results.map((result) => (<div key={result.label} className="p-4 rounded-xl border shadow-sm hover:shadow-md transition-all flex flex-col items-center justify-center text-center bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                          <div className="text-3xl font-extrabold mb-1">{result.value}</div>
                    ...
                    <div className="mt-6 pt-5 border-t border-gray-200 dark:border-gray-600">
                      <div className="text-sm mb-2 text-gray-600 dark:text-gray-400">Project Type:</div>
              ...
              </AnimatePresence>) : (<div className="text-center py-20 text-xl text-gray-600 dark:text-gray-400">
                No testimonials to display yet. Be the first to share your experience!
              </div>)}
          </div>

          
          {testimonials.length > 1 && (<div className="flex justify-center mt-8 space-x-2">
              {testimonials.map((_, index) => (<button key={index} onClick={() => setActiveIndex(index)} className={`w-3 h-3 rounded-full transition-colors ${index === activeIndex ? "bg-blue-600" : "bg-gray-300 dark:bg-gray-600"}`} aria-label={`Go to testimonial ${index + 1}`} aria-current={index === activeIndex}/>))}
            </div>)}
        </div>
        ...
        <div className="mt-8">
          ...
          <div ref={scrollRef} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 overflow-hidden">
            {companies.map((company, index) => (<div key={index} className="rounded-xl p-5 flex items-center justify-center h-24 border shadow-sm hover:shadow-md transition-all duration-300 bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                <div className="flex items-center">
