...
const TiersTab = ({ refreshKey }: {
    ...
    return (<Card className="">
      ...
      <CardContent className="space-y-3 mb-3">
        {tiers.map((tier) => (<div key={tier.id} className="grid grid-cols-3  rounded-lg items-center gap-4 py-4">
            <div className="text-center font-medium">{tier.name}</div>
            <ul className="flex flex-col items-center space-y-2">
              {tier.features.map((feature, idx) => (<li key={idx} className="flex items-center text-sm">
            ...
            <div className="flex flex-col items-center space-y-2">
              <Input type="number" min="0" value={editedPrices[tier.id] ?? tier.price} onChange={(e) => setEditedPrices({
                ...editedPrices,
