...
const NavbarSection: React.FC<NavbarProps> = ({ scrollTo }) => {
        ...
        >
          ...
          <ChevronDown
            className={`w-4 h-4 opacity-80 transition-transform duration-300 ${
              isOpen ? "rotate-180 text-[#D85C2C]" : "rotate-0 group-hover:text-[#D85C2C]"
            }`}
          />
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="absolute top-full left-0 pt-0 w-64 z-50"
            >
              <div className="relative bg-white border border-gray-100 dropdown-shadow p-0 rounded-none">
                {items.map((item) => (
                  <motion.button
                    key={item}
  ...
  // ✅ JTech Logo
  const JTechAILogo = () => (
    <svg width="160" height="34" viewBox="0 0 135 36" fill="none" xmlns="http://www.w3.org/2000/svg" className="transition-all duration-200 hover:opacity-90">
      ...
      <text x="45" y="24" fontFamily="Outfit" fontWeight="800" fontSize="22" fill={THEME.NAVY}>JTech</text>
