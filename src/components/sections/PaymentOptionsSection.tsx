...
const PaymentMethod = ({ method, index }) => (
  <motion.div
    // Card container styles - Engineering Look (Square, white, subtle border)
    className="group bg-white rounded-sm border border-gray-200 shadow-sm p-6 flex flex-col items-center justify-center h-32 cursor-pointer hover:border-[#00356B] hover:shadow-md transition-all duration-300 relative overflow-hidden"
    initial={{ opacity: 0, y: 15 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true }}
    transition={{ delay: index * 0.1, duration: 0.4 }}
  >
    {/* Active Checkmark (Hidden by default, visible on hover idea) */}
    ...
    {/* Label */}
    <span className="mt-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-[#00356B] transition-colors">
        {method.name}
    </span>
  </motion.div>
);
    ...
    <>
      ...
      >
        <div className="max-w-5xl mx-auto">
          ...
          >
            {/* Badge 1 */}
            <div className="flex items-center gap-3 px-6 py-3 bg-white rounded-sm border border-gray-200 shadow-sm">
