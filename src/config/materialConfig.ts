...
// Material validation error interface
...
// Material price interface
export interface MaterialPrice {
  basePrice: number;
  unit: string;
  region: string;
  lastUpdated: Date;
  variations?: { [key: string]: number }; // Price adjustments for variations
  marketFactors?: {
...
// Material configuration validator
...
// Material price result interface
...
// Material price calculator with market factors
export function calculateMaterialPrice(
  ...
  // Apply urgency factor if specified
  ...
  if (basePrice.variations && Object.keys(basePrice.variations).length > 0) {
    // Use the highest variation adjustment
...
// Function to check if the price is within expected range
export function validateMaterialPrice(
  ...
  // Default price ranges for common materials (example values)
  const defaultPriceRanges: { [key: string]: { min: number; max: number } } = {
    concrete: { min: 80, max: 200 }, // per m³
    masonry: { min: 30, max: 100 }, // per m²
    steel: { min: 0.8, max: 2.5 }, // per kg
    cement: { min: 500, max: 1000 }, // per tonne
    sand: { min: 20, max: 50 }, // per tonne
...
// Utility function to get required materials for a specific material type
...
export function validateProjectSpecifications(
  ...
  if (specifications?.mixRatio) {
    // Validate mix ratio format and values
...
export const MaterialConfigurations: { [key: string]: MaterialConfig } = {
  concrete: {
    type: "concrete",
    ...
    properties: {
      ...
      mixRatio: "1:2:4", // Default mix ratio
    },
    relationships: [
      {
        material: "Formwork",
  ...
  masonry: {
    ...
    defaultUnit: "m²",
    ratios: {
      // Default ratios for 1:6 mortar mix
      stones: 16,
      cement: 0.4,
      sand: 0.07,
    },
    properties: {
      ...
      mixRatio: "1:6", // Default mortar mix ratio
      thickness: 0.012, // Default plaster thickness in meters
    },
    relationships: [
      {
        material: "DPC",
  ...
  roofing: {
    ...
    defaultUnit: "m²",
    ratios: {
      tiles: 13.5, // tiles per m²
      battens: 3.2, // m per m²
      nails: 0.15, // kg per m²
      underlay: 1.1, // m² per m² (including overlaps)
    },
    properties: {
      requirements: [
  ...
  finishes: {
    ...
    ratios: {
      ...
      primer: 0.2, // liters per m²
      filler: 0.1, // kg per m²
      sandpaper: 0.2, // sheets per m²
    },
    properties: {
      requirements: [
        "Surface preparation",
...
export function getMaterialConfig(
  ...
  // If we have quote data, update config with project-specific values
  if (quoteData) {
    ...
    // Calculate ratios based on project specifications
...
export function getMaterialBreakdown(
  ...
  // Use calculated ratios if available, otherwise use default ratios
  ...
  const breakdown = Object.entries(ratios).map(([material, ratio]) => {
    ...
    // Add warning for unusually large quantities
...
function isUnusualQuantity(material: string, quantity: number): boolean {
  // Define reasonable ranges for different materials
  const ranges: { [key: string]: { min: number; max: number } } = {
    cement: { min: 0.1, max: 1000 }, // bags
    sand: { min: 0.1, max: 100 }, // tonnes
    ballast: { min: 0.1, max: 100 }, // tonnes
    water: { min: 1, max: 10000 }, // litres
...
function getVariationsForMaterial(
  ...
  const materialVariations: { [key: string]: { [key: string]: string[] } } = {
    concrete: {
      cement: [
...
export function calculateMaterialQuantities(
  ...
  baseQuantity: number,
  quoteData?: QuoteCalculation,
  customRatios?: { [key: string]: number }
): {
