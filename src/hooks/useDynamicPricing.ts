...
export const useDynamicPricing = () => {
    ...
    const updateMaterialPrice = async (materialName: string, materialId: string, region: string, newData: any, index?: number | string) => {
        if (!user)
        ...
        try {
            ...
            if (Array.isArray(updatedType)) {
                ...
                else if ((materialName === "Bricks" || materialName.includes("Block")) &&
                    typeof index === "number") {
                    updatedType[index] = {
                        ...updatedType[index],
                        price_kes: newData,
    ...
    const getEffectiveMaterialPrice = (materialId: string, region: string, userOverride: any, materialBasePrices: any[], regionalMultipliers: {
        region: string;
        multiplier: number;
    }[]) => {
        ...
        if (Array.isArray(cloned.type)) {
            cloned.type = cloned.type.map((item) => {
                ...
                if (updated.price_kes && typeof updated.price_kes === "object") {
                    updated.price_kes = Object.fromEntries(Object.entries(updated.price_kes).map(([size, price]) => [
                        size,
    ...
    const getEffectiveMaterialPriceSingle = (materialId: string, region: string) => {
        const userOverride = userMaterialPrices.find((p) => p.material_id === materialId && p.region === region);
        if (userOverride) {
