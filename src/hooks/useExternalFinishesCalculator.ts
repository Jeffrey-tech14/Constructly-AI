...
const EXTERNAL_PLASTER_THICKNESS_MM = 25; // HARDCODED - Do not change

export interface ExternalFinishCalculation {
  id: string;
  material: string;
  quantity: number;
...
export interface ExternalFinishTotals {
  totalArea: number;
...
/**
 * Get walling material unit rate from materialPrices for external finishes
 */
function getWallingRate(finish: FinishElement, prices: any[]): number {
  ...
  const allMaterials = [
    ...(wallingCategory.type?.externalWallingMaterials || []),
...
export default function useExternalFinishesCalculator(
  finishes: FinishElement[],
  materialPrices: any[],
  quote: any,
  externalWallArea: number = 0,
  wallPointingEnabled: boolean = false,
) {
  const [calculations, setCalculations] = useState<ExternalFinishCalculation[]>([]);
  const [totals, setTotals] = useState<ExternalFinishTotals>({
    totalArea: 0,
    totalQuantity: 0,
  ...
  // Adjust plaster quantity for fixed 25mm thickness
  const adjustPlasterQuantity = useCallback(
    (finish: FinishElement): FinishElement => {
      if (finish.material?.toLowerCase().includes("plaster") && externalWallArea > 0) {
        // External plaster volume = area × 0.025m (25mm)
        const plasterVolume = externalWallArea * (EXTERNAL_PLASTER_THICKNESS_MM / 1000);
        return {
          ...finish,
          quantity: plasterVolume,
          specifications: {
            ...finish.specifications,
            thickness: EXTERNAL_PLASTER_THICKNESS_MM,
          },
        };
      }
      return finish;
  ...
  const filteredFinishes = finishes.filter((f) => {
    if (wallPointingEnabled && f.material?.toLowerCase() === "plaster") {
      return false;
    }
    return true;
  });
  ...
  const calculateFinish = useCallback(
    (finish: FinishElement): ExternalFinishCalculation => {
  ...
  const calculateAll = useCallback(() => {
    const calculated = filteredFinishes.map(calculateFinish);
    ...
    setTotals(newTotals);
  }, [filteredFinishes, calculateFinish, getWastagePercentage]);
  ...
  }, [filteredFinishes, calculateAll, getWastagePercentage]);
  ...
  return {
    ...
    externalPlasterThickness: EXTERNAL_PLASTER_THICKNESS_MM,
  };
}
