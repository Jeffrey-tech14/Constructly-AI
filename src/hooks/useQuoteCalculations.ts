...
export const useQuoteCalculations = () => {
    ...
    const fetchMaterials = useCallback(async () => {
        ...
        const merged = baseMaterials.map((material) => {
            ...
            const multiplier = regionalMultipliers.find((r) => r.region === userRegion)?.multiplier ||
                1;
    ...
    const fetchServices = useCallback(async () => {
        ...
        const { data: overrides, error: overrideError } = await supabase
            ...
            .eq("user_id", profile.id);
        if (baseError)
            console.error("Base services error:", baseError);
        if (overrideError)
            console.error("Overrides error:", overrideError);
    ...
    const fetchEquipment = useCallback(async () => {
        ...
        const { data: overrides, error: overrideError } = await supabase
            ...
            .eq("user_id", profile.id);
        if (baseError)
            console.error("Base equipment error:", baseError);
        if (overrideError)
            console.error("Overrides error:", overrideError);
    ...
    const fetchTransportRates = useCallback(async () => {
        ...
        const { data: overrides, error: overrideError } = await supabase
            ...
            .eq("user_id", profile.id);
        if (baseError)
            console.error("Base transport rates error:", baseError);
        if (overrideError)
            console.error("Overrides error:", overrideError);
        const allRegions = [
            "Nairobi",
    ...
    const calculateQuote = async (params: QuoteCalculation): Promise<CalculationResult> => {
        ...
        setLoading(true);
        try {
            const { include_wastage, boqData, equipment, services, subcontractors, distance_km, labor_percentages, overhead_percentages, profit_percentages, contingency_percentages, permit_cost, contract_type, preliminaries, } = params;
            const calculatePreliminariesTotal = (): number => {
                if (!Array.isArray(preliminaries))
                    return 0;
            ...
            const transportCost = (() => {
                ...
                if (!rateForRegion) {
                    ...
                    return (distance_km * defaultTransportRate.cost_per_km +
            ...
            const overheadAmount = (subtotalBeforeExtras *
                (parseFloat(overhead_percentages.toString()) || 0)) /
                100;
            const contingencyAmount = (subtotalBeforeExtras *
            ...
            return {
                ...
                regional_multiplier: regionalMultipliers.find((r) => r.region === (profile?.location || "Nairobi"))?.multiplier || 1,
