...
export interface Quote {
  ...
  custom_specs?: string;
  status: string;
  paid?: boolean; // New field for payment status
  ...
  selected_equipment?: any[]; // Equipment selected for the quote
  selected_services?: any[]; // Services selected for the quote
  contract_type?: "full_contract" | "labor_only"; // Type of contract
  created_at: string;
  updated_at: string;
  ...
  const fetchQuotes = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        const quotesData: Quote[] = (data || []).map((item: any) => ({
          ...item,
    ...
    [user, profile?.is_admin, isInitialLoad]
  );
