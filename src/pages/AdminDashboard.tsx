...
import { Users, DollarSign, Activity, TrendingUp, UserX, UserCheck, Crown, Ban, Shield, Search, MoreHorizontal, Settings, Star, ThumbsUp, Shell, Bitcoin, BitcoinIcon, Coins, } from "lucide-react";
...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const totalRevenue = activeQuotes.reduce((sum, quote) => sum + (quote.profit_amount || 0), 0);
    ...
    useEffect(() => {
        ...
        const tiersChannel = supabase
            ...
            .on("postgres_changes", { event: "*", schema: "public", table: "tiers" }, (payload) => {
    ...
    return (<div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
        <div className="mb-8">
          <div className="flex items-center sm:text-3xl text-2xl font-bold bg-gradient-to-r from-blue-900 via-indigo-600 to-indigo-900 dark:from-white dark:via-white dark:to-white bg-clip-text text-transparent">
            <Crown className="sm:w-8 sm:h-8 mr-2 text-blue-900 dark:text-white"/>
            Admin Dashboard
          </div>
        ...
        <Tabs defaultValue="users" className="space-y-6">
            ...
            <Card className="">
              ...
              <CardContent>
                ...
                <div className="space-y-3 md:space-y-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {filteredUsers.map((user) => (<Card key={user.id} className={`p-3 md:p-4 bg- relative ${user.id === profile.id
                ? "ring-2 ring-primary dark:ring-white"
                : ""}`}>
                      <div>
                        {user.id === profile.id && (<Badge className="absolute -top-2 md:-top-3 transform translate-x-2 bg-primary text-white dark:bg-white dark:text-primary rounded-full px-3 md:px-4 py-0.5 md:py-1 text-xs md:text-sm">
                            Me
                          </Badge>)}
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                          ...
                          <div className="flex items-start md:items-center space-x-3 md:space-x-4 flex-1 min-w-0">
                            ...
                            <div className="min-w-0 space-y-1">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {user.is_admin && (<Badge variant="destructive" className="text-xs md:text-sm">
                                    <Crown className="w-3 h-3 mr-1"/>
                                    Admin
                                  </Badge>)}
                              </div>
                              ...
                              <div className="flex flex-wrap items-center gap-2 md:gap-4">
                                <Badge variant="secondary" className="text-xs md:text-sm bg-transparent text-inherit border-transparent hover:bg-transparent hover:text-inherit hover:border-transparent focus:bg-transparent focus:text-inherit focus:border-transparent active:bg-transparent active:text-inherit active:border-transparent">
                          ...
                          <div className="flex items-center justify-start ml-5 md:justify-normal space-x-2">
                            <Select value={user.tier} onValueChange={(value) => updateUserTier(user.id, value)}>
                              <SelectTrigger className=" md:w-32 text-xs md:text-sm">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="Free" className="text-xs md:text-sm">
                                  Free
                                </SelectItem>
                                <SelectItem value="Intermediate" className="text-xs md:text-sm">
                                  Intermediate
                                </SelectItem>
                                <SelectItem value="Professional" className="text-xs md:text-sm">
                                  Professional
                                </SelectItem>
                              </SelectContent>
                            </Select>
          ...
          <TabsContent value="analytics" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {quotes.length > 0 && (<Card className="">
                  <CardHeader>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <BarChart data={monthlyData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                        ...
                        <Tooltip labelClassName="text-black rounded-lg" formatter={(value: number) => formatCurrency(value)}/>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={tierData} cx="50%" cy="50%" outerRadius={100} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                        {tierData.map((entry, index) => (<Cell key={`cell-${index}`} fill={["#3b82f6", "#10b981", "#f59e0b", "#ef4444"][index % 4]}/>))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                  </ResponsiveContainer>
