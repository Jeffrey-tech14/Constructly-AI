...
import { DollarSign, FileText, Eye, BarChart, Settings, Calendar as CalendarIcon, TrendingUp, Users, Star, Clock, CheckCircle, Workflow, PersonStanding, PersonStandingIcon, LayoutDashboard, } from "lucide-react";
...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const recentQuotes = quotes
                ...
                .slice(0, 5);
            const allQuotes = quotes.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
            setDashboardData({
                totalQuotesValue,
                completedProjects,
                activeProjects,
    ...
    if (quotesLoading) {
        return (<div className="min-h-screen grid flex grid-rows-2 items-center justify-center">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
      </div>);
    }
    return (<div className="min-h-screen inset-0 scrollbar-hide">
      <div className="max-w-7xl scrollbar-hide mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-4 animate-fade-in">
          <div className="sm:flex flex-1 justify-between items-center">
            ...
            <Button onClick={() => setShowCalculator(true)} className="text-sm px-5 py-4 rounded-xl sm:mt-0 mt-4 shadow-lg bg-primary animate-bounce-gentle hover:shadow-2xl transition-transform duration-300 text-white">
              ⚡Quick Calculator
            </Button>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="animate-fade-in">
          <TabsList className={`grid w-full ${profile.tier === "Professional" ? "grid-cols-3" : "grid-cols-2"} ${profile.tier === "Free" ? "grid-cols-1" : "grid-cols-3"} mb-6`}>
            ...
            {profile.tier === "Professional" && (<TabsTrigger value="reports" className="flex items-center">
                <TrendingUp className="w-4 h-4 mr-2"/>
                Reports
              </TabsTrigger>)}
            {profile.tier !== "Free" && (<TabsTrigger value="calendar" className="flex items-center">
          ...
          <TabsContent value="overview" className="space-y-6 animate-fade-in">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-3 ${profile.tier === "Free" ? "lg-grid-cols-3" : "lg:grid-cols-3"} gap-6`}>
              <Card className=" card-hover">
                ...
                <CardContent>
                  <div className="sm:text-2xl text-lg font-bold">
                    KSh{" "}
                    {formatCurrency(dashboardData.totalQuotesValue).toLocaleString()}
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className=" lg:col-span-2">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {dashboardData.recentQuotes.length > 0 ? (dashboardData.recentQuotes.map((quote) => (<div key={quote.id} className="flex items-center justify-between p-4 dark:bg-gray-900 border border-gray-100 dark:border-gray-600/40 rounded-lg hover:shadow-lg transition-shadow">
              ...
              {profile.tier !== "Free" && (<div className="space-y-6">
                  <Card className="">
                    <CardHeader>
                      <CardTitle className="flex items-center">
                    ...
                    <CardContent>
                      {dashboardData.upcomingEvents.length > 0 ? (<div className="space-y-3">
                          {dashboardData.upcomingEvents.map((event) => (<div key={event.id} className="p-3 border-t dark:border-white/20 rounded">
          ...
          {profile.tier !== "Free" && (<TabsContent value="reports">
              <Reports />
            </TabsContent>)}
