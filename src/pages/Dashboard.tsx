...
// Assuming useQuotes, useClientReviews, and useCalendarEvents still exist but we rely on local fetch
...
  const { loading: quotesLoading } = useQuotes(); // We keep this if other parts rely on it
...
  });
  
  // 1. Introducing a local state for dashboard data fetching
...
      // Only set loading to true on the very first load or explicit refresh
...
    
    fetchAndSet();
  }, [user, location.key, profile?.id]); // Added profile.id dependency for initial fetch
  
...
  };
  
  // 3. Use the correct loading state
