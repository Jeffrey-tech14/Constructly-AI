...
import { CreditCard, Smartphone, Check, ArrowLeft, Shield, Briefcase, Coins, Loader2, Shell, Crown, Copy, ArrowBigUp, Monitor, } from "lucide-react";
    ...
    return (<div className="min-h-screen animate-fade-in">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                {tiersLoading && (<div className="space-y-4">
                    {[1, 2, 3].map((i) => (<div key={i} className="h-32 dark:bg-gray-900 bg-gray-300 rounded-md animate-pulse"></div>))}
                  </div>)}

                {tiersError && (<div className="p-4 rounded-md bg-destructive text-white">
            ...
            return (<div key={plan.id} className={`relative overflow-hidden rounded-2xl border-2 p-6 transition-all duration-300
        ${isSelected
        ...
        ${isCurrent
                    ...
                    : "cursor-pointer"}`} onClick={() => !isCurrent && setSelectedPlan(plan.name)}>
                      
                      {plan.popular && (<div className="absolute -right-10 top-4 rotate-45 z-20">
                      ...
                      <div className="mb-6 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div className={`flex h-12 w-12 items-center justify-center rounded-full ${plan.name === "Free"
                        ...
                        : "bg-purple-100 text-purple-700"}`}>
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {isCurrent && (<Badge className="bg-green-600 text-white text-xs px-2 py-0.5">
                    ...
                    : `KSh ${plan.price.toLocaleString()}`}
                              ...
                              {plan.price > 0 && (<span className="ml-1 text-sm font-medium text-muted-foreground">
                                  /month
                                </span>)}
                            </div>
                          </div>
                        ...
                        {isSelected && !isCurrent && (<div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary dark:bg-white">
                      ...
                      {!isCurrent ? (<Button className={`w-full rounded-full py-2   ${isSelected
                        ? "bg-primary text-white hover:bg-primary/90"
                        : "bg-muted text-foreground hover:bg-muted/80"}`}>
                          {isSelected ? "Selected" : "Select Plan"}
                        </Button>) : (<div className="w-full rounded-full bg-green-100 py-2 text-center text-sm font-semibold text-green-800">
                          Active Plan
                        </div>)}
                    </div>);
        })}
          ...
          <div className="space-y-8">
            ...
            <Card className="shadow-sm overflow-hidden">
              <CardHeader className="bg-muted/40 border-b px-6 py-4">
                <CardTitle className="flex items-center text-base font-semibold">
                  <svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-5 w-5 text-primary dark:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <CardContent className="pt-5">
                <div className="grid gap-4">
                  {paymentMethods.map((method) => (<div key={method.id} className={`group relative flex border-border border-primary/10 dark:border-white/10 border-primary/10 items-center justify-between rounded-xl border-2 p-4 transition-all ${paymentMethod === method.id
                ? "border-primary dark:border-white/80 bg-primary/5 shadow-sm"
                : "border-border hover:border-primary/30 dark:hover:border-white/30 hover:bg-accent/50 cursor-pointer"}`} onClick={() => setPaymentMethod(method.id)}>
                      <div className="flex items-center gap-3">
                        <div className={`flex h-12 w-12 items-center justify-center rounded-lg ${paymentMethod === method.id
                ? "bg-primary/10"
                : "bg-muted"}`}>
                          {method.icon}
                        </div>
                      ...
                      {paymentMethod === method.id && (<div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary dark:bg-white shadow-sm">
            ...
            {selectedPlanDetails && (<Card className="shadow-sm overflow-hidden">
                ...
                : `KSh ${selectedPlanDetails.price.toLocaleString()}`}
                        ...
                        {selectedPlanDetails.price > 0 && (<div className="text-sm text-muted-foreground">
                            per month
                          </div>)}
                      </div>
                    </div>
            ...
            <div className="space-y-3">
              <Button className="w-full h-12 text-base text-white font-semibold rounded-xl" onClick={() => {
            ...
            selectedPlan === profile?.tier}>
                {processing ? (<>
                    ...
                    Current Plan
                  </>) : selectedPlanDetails?.price === 0 ? ("Downgrade to Free") : (<>
