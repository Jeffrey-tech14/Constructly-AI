import { Document, Paragraph, Table, TableRow, TableCell, WidthType, AlignmentType, HeadingLevel, TextRun, BorderStyle, convertInchesToTwip, PageBreak, ShadingType, Footer, PageNumber, Header, SectionType, ExternalHyperlink, ImageRun, Media, Packer, } from "docx";
...
const createMaterialsScheduleTable = (materials: any[]): Table => {
    ...
    const totalAmount = materials.reduce((sum, material) => sum + (material.amount || 0), 0);
    rows.push(new TableRow({
        children: [
            new TableCell({
            ...
            new TableCell({
                children: [
                    new Paragraph({
                        children: [
                            new TextRun({
                                text: `TOTAL FOR PRELIMINARIES: ${formatCurrency(preliminariesTotal)}`,
                                bold: true,
                            }),
                        ],
                        alignment: AlignmentType.RIGHT,
...
const createCostBreakdownTable = (quote: any, isClientExport: boolean): Table => {
    const rows: TableRow[] = [
        new TableRow({
            children: [
                new TableCell({
                ...
                new TableCell({
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `Overhead (${quote.percentages?.[0]?.overhead || 0}%)`,
                                }),
                            ],
                        }),
                    ],
                ...
                new TableCell({
                    children: [
                        new Paragraph({
                            children: [
                                new TextRun({
                                    text: `Contingency (${quote.percentages?.[0]?.contingency || 0}%)`,
                                }),
                            ],
                        }),
                    ],
...
export const generateQuoteDOCX = async ({ quote, projectInfo, isClientExport = false, }: DOCXExportOptions) => {
    const materials = extractMaterialsFromQuote(quote);
    const boqData = quote.boq_data || [];
    const preliminariesData = quote.preliminaries || [];
    const nonEmptySections = boqData.filter((section: any) => section.items && Array.isArray(section.items) && section.items.length > 0);
        ...
        sections: [
            {
                ...
                children: [
                    ...
                    ...(!isClientExport
                        ? [
                            new Paragraph({
                                children: [
                                    new TextRun({
                                        text: `• Includes ${quote.percentages?.[0]?.profit || 0}% profit margin`,
                                    }),
                                ],
                                spacing: { after: 100 },
                            }),
                            new Paragraph({
                                children: [
                                    new TextRun({
                                        text: `• Includes ${quote.percentages?.[0]?.contingency || 0}% contingency`,
                                    }),
                                ],
                                spacing: { after: 100 },
                            }),
                    ...
                    new Paragraph({
                        children: [
                            new TextRun({
                                text: `• Valid for 30 days from ${projectInfo.date || "date of issue"}`,
                            }),
                        ],
                        spacing: { after: 100 },
                    }),
    ...
    const fileName = `${isClientExport ? "Client" : "Contractor"}_Quote_${(quote.title || "Quote").replace(/\s+/g, "_")}.docx`;
