...
export const generateQuoteExcel = async ({ quote, isClientExport = false, }: ExcelExportOptions) => {
    ...
    if (!isClientExport) {
        summaryData.push(["Overhead", quote.overhead_amount], ["Contingency", quote.contingency_amount], ["Profit", quote.profit_amount]);
    }
    else {
    ...
    if (headerCell)
        (headerCell as any).s = { font: { bold: true } };
    ...
    if (!isClientExport) {
        ...
        if (materials && materials.length > 0) {
            ...
            const materialsTotal = materials.reduce((sum: number, m: any) => sum + m.amount, 0);
            materialsData.push([
                "",
    ...
    const fileName = `${isClientExport ? "Client" : "Contractor"}_Quote_${quote.title.replace(/\s+/g, "_")}.xlsx`;
