...
import { AdvancedMaterialExtractor, MaterialSchedule, } from "./advancedMaterialExtractor";
...
const exportBOQPDF = async (boqData: BOQSection[], projectInfo: ProjectInfo, preliminaries: any[], quote: any, isClientExport: boolean = false): Promise<boolean> => {
    try {
        ...
        if (quote?.concrete_materials ||
            ...
            quote?.rooms) {
            try {
                const rawSchedule: MaterialSchedule = await AdvancedMaterialExtractor.extractWithGemini(quote);
                materialSchedule = MaterialConsolidator.consolidateAllMaterials(Object.values(rawSchedule).flat());
            }
            catch (error) {
                console.warn("AI extraction failed, falling back to local extraction:", error);
