...
export class MaterialConsolidator {
    static consolidateAllMaterials(materials: CategorizedMaterial[]): ConsolidatedMaterial[] {
        ...
        materials.forEach((material) => {
            if (material.materialBreakdown) {
                material.materialBreakdown.forEach((breakdown) => {
                    const key = `${this.cleanDescription(breakdown.material)}_${breakdown.unit}_${breakdown.category}`.toLowerCase();
            ...
            const key = `${this.cleanDescription(material.description)}_${material.unit}_${material.category}`.toLowerCase();
            if (materialMap.has(key)) {
                const existing = materialMap.get(key)!;
    ...
    private static generateConsolidatedDescription(material: ConsolidatedMaterial): string {
