...
const loaderCode = '<div className="min-h-screen bg-[#111418] flex flex-col items-center justify-center p-4">\\n' +
...
content = content.replace(
  /<div className="min-h-screen flex items-center justify-center">[\s\S]*?<Loader2 className="animate-spin rounded-full h-8 w-8" \/>[\s\S]*?<\/div>/g,
  loaderCode
);
